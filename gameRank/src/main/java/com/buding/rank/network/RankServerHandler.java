...
@Sharable
public class RankServerHandler extends SimpleChannelInboundHandler<Message> {
	 ...
	 * 
	 */
	private static final AttributeKey<RankSession> SESSION = AttributeKey.newInstance("NettyHandler.RankSessionKey");
	...
	public void channelRead0(ChannelHandlerContext ctx, Message msg) throws Exception {
		...
		RankSession session = (RankSession) ctx.attr(SESSION).get();
...
//	}
	...
	public void initWhenConnected(ChannelHandlerContext ctx) {
		...
		rankSessionManager.put2AnonymousList(session);
		ctx.attr(SESSION).set(session);
	}
	
	public void onSessionClosed(RankSession session, ChannelHandlerContext ctx) {
		rankSessionManager.schedule2Remove(session);
	}
	
	public void cleanWhenClosed(ChannelHandlerContext ctx) {
		RankSession session = ctx.attr(SESSION).get();
		if (session != null) {

 ...
 * @Description: 游戏状态
 */
public class DDZStateRun extends PokerStateCommon {
    ...
    public void onNet(NetEvent event) {
        try {
            ...
            switch (p.getOperType()) {
                case DDZGameOperPlayerActionSyn: {
                    DDZ.DDZGameOperPlayerActionSyn.Builder gb = DDZ.DDZGameOperPlayerActionSyn.newBuilder();
    ...
    public void onEnter() {
        // 发送公共信息
        DDZPokerPushHelper.pushPublicInfoMsg2All(this.mDesk, this.mGameData);
    ...
    public void onDeskTimer() {
        ...
        if (!this.mTimerForStateChange) {
            ...
            if (state == PokerConstants.GAME_TABLE_STATE_PLAYING) {// 玩家玩牌中,状态为4
                ...
                game_over();
                return;
            } else if (state == PokerConstants.GAME_TABLE_STATE_RE_SEND_CARD) {
                re_send();
                return;
            }
            checkFinish();
    ...
    private void re_send() {
        this.mGameData.mGameWin.reset();
        this.mGameData.Reset();//重置桌子参数
        this.mCardDealer.dealCard();
        // 设置从谁开始喊地主
        this.mCardLogic.selectBanker(this.mGameData, this.mDesk);
        // 发牌

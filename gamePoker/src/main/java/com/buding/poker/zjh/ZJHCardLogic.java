...
public class ZJHCardLogic implements ICardLogic<MJDesk<byte[]>> {
...
//        }
     ...
     * 是否能赢
     */
    private ZJHBiPaiResult biPai(GameData gameData, MJDesk<byte[]> desk, ZJHGameOperPlayerActionSyn.Builder msg) {
        ...
        if(wangNum+biPaiWangNum == 0){
            ...
            //双方各拼凑出最大的来比较
            result.cardsInRealMax = getCardsInRealMax(result.cardsInReal,desk);
            result.biPaiCardsInRealMax = getCardsInRealMax(result.biPaiCardsInReal,desk);
    ...
    private boolean isEqualValue(List<Byte> cardsInRealMax, List<Byte> biPaiCardsInRealMax) {
    ...
    private List<Byte> getCardsInRealMax(Map<List<Byte>, Integer> cardsInReal,MJDesk<byte[]> desk) {
...
//        }
    ...
    private void biPaiWithoutWang(MJDesk<byte[]> desk, ZJHBiPaiResult result) {
        ...
        if(type == biPaitype){//两人牌型相同
            ...
            //考虑下地龙
            if(desk.canDiLong() && (type == PokerConstants.ZJH_CARDTYPE_SHUN_ZI || type == PokerConstants.ZJH_CARDTYPE_SHUN_JIN)){
            ...
            //如果分别是金花,顺子
            if(desk.canShunThanJin() && type == PokerConstants.ZJH_CARDTYPE_JIN_HUA
                    && biPaitype == PokerConstants.ZJH_CARDTYPE_SHUN_ZI){
                result.winnerPos = result.biPaipos;
            }

            if(desk.can235ThanBaoZi() && type == PokerConstants.ZJH_CARDTYPE_BAO_ZI
                    && biPaitype == PokerConstants.ZJH_CARDTYPE_DAN){
                if(is235(result.biPaiCards)) result.winnerPos = result.biPaipos;
            }

            if(desk.can235ThanAAA() && type == PokerConstants.ZJH_CARDTYPE_BAO_ZI
                    && biPaitype == PokerConstants.ZJH_CARDTYPE_DAN){
                if(is235(result.biPaiCards) && isAAA(result.cards)) result.winnerPos = result.biPaipos;
            }
        } else {//比牌人牌型<被比牌人
            result.winnerPos = result.biPaipos;
            if(desk.canShunThanJin() && type == PokerConstants.ZJH_CARDTYPE_SHUN_ZI
                    && biPaitype == PokerConstants.ZJH_CARDTYPE_JIN_HUA){
                result.winnerPos = result.pos;
            }

            if(desk.can235ThanBaoZi() && type == PokerConstants.ZJH_CARDTYPE_DAN
                    && biPaitype == PokerConstants.ZJH_CARDTYPE_BAO_ZI){
                if(is235(result.cards)) result.winnerPos = result.pos;
            }

            if(desk.can235ThanAAA() && type == PokerConstants.ZJH_CARDTYPE_DAN
                    && biPaitype == PokerConstants.ZJH_CARDTYPE_BAO_ZI){
    ...
    //再变顺子(与较大的一个变同色)
    private Map<List<Byte>,Integer> getCardsInReal(List<Byte> cards,boolean canDiLong) {
    ...
    private List<Byte> convertShunZi(List<Byte> temp, List<Byte> realCards, boolean canDiLong) {
    ...
    private void convertBiggerCard(List<Byte> temp, List<Byte> realCards) {
    ...
    private void clearTempCard(List<Byte> cards, List<Byte> temp) {
        temp.clear();
        temp.addAll(cards);
        temp.remove(Byte.valueOf(PokerConstants.POKER_CODE_DA_WANG));
        temp.remove(Byte.valueOf(PokerConstants.POKER_CODE_XIAO_WANG));
    ...
    private void convertA(List<Byte> temp, List<Byte> realCards) {
        byte maxCardColor = getMaxCardColor(realCards);
        temp.add((byte)(maxCardColor << 4 | 14));
    }

    private byte getMaxCardValue(List<Byte> realCards) {
        List<Byte> list = ZJHRule.modular(realCards);
        if(realCards.size() == 3){
            return list.get(2);
    ...
    private byte getMaxCardColor(List<Byte> realCards) {
        if(realCards.size() == 3){
    ...
    private boolean isAAA(List<Byte> cards) {
        List<Byte> list = ZJHRule.modular(cards);
        return list.size() == 3 && list.get(0) == 14 && list.get(1) == 14 && list.get(2) == 14;
    }

    private boolean is235(List<Byte> cards) {
        List<Byte> list = ZJHRule.modular(cards);
        return list.size() == 3 && list.get(0) == 2 && list.get(1) == 3 && list.get(2) == 5;
    }

    private boolean compareThreeCard(List<Byte> cards, List<Byte> biPaiCards, int type) {
    ...
    private int getType(List<Byte> cardsInHand) {
    ...
    private boolean checkDuiZi(List<Byte> cardsInHand) {
    ...
    private boolean checkShunZi(List<Byte> list) {
    ...
    private boolean checkJinHua(List<Byte> cardsInHand) {
    ...
    private boolean checkShunJin(List<Byte> list) {
        int card1 = list.get(0) & 0x0f;
        int card2 = list.get(1) & 0x0f;
        int card3 = list.get(2) & 0x0f;
    ...
    private boolean checkBaoZi(List<Byte> cardsInHand) {
        int card1 = cardsInHand.get(0) & 0x0f;
        int card2 = cardsInHand.get(1) & 0x0f;
        int card3 = cardsInHand.get(2) & 0x0f;
    ...
    private int getCardType(List<Byte> cardsInHand, MJDesk<byte[]> desk) {
        if(cardsInHand.contains(Byte.valueOf(PokerConstants.POKER_CODE_DA_WANG))
            ...
            return getType(getCardsInRealMax(getCardsInReal(cardsInHand,desk.canDiLong()),desk));
        } else {

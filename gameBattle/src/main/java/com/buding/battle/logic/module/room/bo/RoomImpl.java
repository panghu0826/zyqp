...
//			if(desk.getPlayerCount() == desk.getDeskConfig().seatSizeUpper && !desk.getplayerIdList().contains(ctx.session.userId)){
//				logger.info("act=playerTrySit;error=deskFull;playerId={};deskId={};", ctx.playerId, desk.getDeskID());
//				PushService.instance.pushVipRoomPlayerMsg(desk,ctx.session);
//				return EnterRoomResult.fail("桌子已满");
//			}
			logger.error("desk.isStart()--"+desk.isStart()+"desk.canZhongTuNotEnter()--"+desk.canZhongTuNotEnter());
//			if(desk.isStart() && desk.canZhongTuNotEnter()){
//				logger.info("act=playerTrySit;error=ZHONGTUJINRU;playerId={};deskId={};", ctx.playerId, desk.getDeskID());
//				PushService.instance.pushVipRoomPlayerMsg(desk,ctx.session);
//				return EnterRoomResult.fail("桌子已满");
//			}
			if(desk.getClubId() > 0){
				ClubUser clubUser = ServiceRepo.clubDao.selectClubUser(desk.getClubId(),ctx.playerId);
				if(clubUser == null){
					logger.info("act=playerTrySit;error=notInClub;playerId={};deskId={};", ctx.playerId, desk.getDeskID());
					return EnterRoomResult.fail("不是该俱乐部成员不能进入");
				}
			}
			if(desk.getClubId() >0 && ret.isCreateRoom){
                return EnterRoomResult.ok(desk.getDeskID(), 100);
            }
			int position = desk.playerSitPre(ctx);
			ctx.setDeskId(desk.getDeskID());
			ctx.session.enterDesk(desk);
//			logger.info("ctx.session.getDeskMap().size--"+ctx.session.getDeskMap().size());
	...
	public void playerExit(int playerId, PlayerExitType reason) {
		...
		CommonDesk desk = session.getPlayingOrReadyDesk(playerId);
		if (desk == null) {
			// playerCount--;
	...
	public void onPlayerSit(CommonDesk desk, PlayerInfo player) {
        ...
        if(desk.getClubId() > 0) ServiceRepo.taskPortalService.pushClubRoomModelSyn(BattleConstants.SYN_TYPE_MODIFY,desk.getClubId(),desk.getClubRoomType(),desk.getDeskInfo());
	}
	...
	public void onDeskGameFinish(CommonDesk desk, Game game) {
		if (desk.isAutoChangeDesk()) {
			this.destroyDesk(desk.getDeskID(), DeskDestoryReason.GAME_OVER);
		}
	}
	...
	public void onDeskDestroy(CommonDesk desk) {
		...
		deskCount--;
		if(desk.getClubId() > 0)
			ServiceRepo.taskPortalService.pushClubRoomModelSyn(BattleConstants.SYN_TYPE_DELETE, desk.getClubId(), desk.getClubRoomType(), desk.getDeskInfo());
	}
	...
	public List<DeskModel> getClubJiFenDesk(long clubId) {
		List<DeskModel> list = new ArrayList<DeskModel>();
		for(CommonDesk desk : this.guard.getClubJiFenDeskMap(clubId)) {
			list.add(desk.getDeskInfo());
		}
		return list;
	}
	...
	public List<DeskModel> getClubCommonDesk(long clubId) {
		List<DeskModel> list = new ArrayList<DeskModel>();
		for(CommonDesk desk : this.guard.getClubCommonDeskMap(clubId)) {
			list.add(desk.getDeskInfo());
		}
		return list;
	}
